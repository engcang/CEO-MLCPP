...
namespace ivoxel {
    ...
    class IVox {
       public:
        ...
        struct Options {
            float resolution_ = 0.25;                        // ivox resolution
        ...
        /// get every points
        void GetAllPoints(PointVector& closest_cloud) const;

        /// check collision
        bool IfPointInRadius(const PointType& pt, const double &radius) const;
    ...
    template <int dim, IVoxNodeType node_type, typename PointType>
    void IVox<dim, node_type, PointType>::GetAllPoints(PointVector& all_point_cloud) const{
        if (node_type==IVoxNodeType::DEFAULT){
            for (auto& it : grids_map_) {
                // all_point_cloud.push_back(it.second->second.GetFirstPoint());
                all_point_cloud.push_back(it.second->second.GetEndPoint());
            }
        }
        else if (node_type==IVoxNodeType::PHC){
            for (auto& it : grids_map_) {
                all_point_cloud.push_back(it.second->second.GetCenter());
            }
        }
    }

    template <int dim, IVoxNodeType node_type, typename PointType>
    bool IVox<dim, node_type, PointType>::IfPointInRadius(const PointType &pt, const double &radius) const{
        auto key = Pos2Grid(ToEigen<float, dim>(pt));
        for (const KeyType& delta : nearby_grids_) {
            auto dkey = key + delta;
            auto iter = grids_map_.find(dkey);
            if (iter != grids_map_.end()) {
                if (iter->second->second.IfPointInRadius(pt, radius)){
                    return true;
                }            
            }
        }        
