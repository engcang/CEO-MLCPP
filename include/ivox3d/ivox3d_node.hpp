...
namespace ivoxel {
    ...
    class IVoxNode {
       public:
        ...
        bool IfPointInRadius(const PointT& pt, const double &radius) const;

        PointT GetFirstPoint() const;

        PointT GetEndPoint() const;
    ...
    template <typename PointT, int dim>
    bool IVoxNode<PointT, dim>::IfPointInRadius(const PointT& pt, const double &radius) const{
        PointT ptf = points_.front();
        PointT ptb = points_.back();
        if( sqrt( (pt.x-ptb.x)*(pt.x-ptb.x)+(pt.y-ptb.y)*(pt.y-ptb.y)+(pt.z-ptb.z)*(pt.z-ptb.z) ) < radius ){
            return true;
        }
        else if ( sqrt( (pt.x-ptf.x)*(pt.x-ptf.x)+(pt.y-ptf.y)*(pt.y-ptf.y)+(pt.z-ptf.z)*(pt.z-ptf.z) ) < radius){
            return true;
        }
        return false;
    }
    ...
    template <typename PointT, int dim>
    PointT IVoxNode<PointT, dim>::GetFirstPoint() const{
        return points_.front();
    }

    template <typename PointT, int dim>
    PointT IVoxNode<PointT, dim>::GetEndPoint() const{
        return points_.back();
    }
    ...
    template <typename PointT, int dim>
    PointT IVoxNodePhc<PointT, dim>::GetCenter() const{
        return center_;
    }
